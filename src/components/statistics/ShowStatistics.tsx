...
export default class ShowStatistics extends Component<any, any> {
  ...
  render() {
    ...
    if (
      ...
      this.state.j1best === this.state.j2best ||
      this.state.j1best === this.state.j3best
    ) {
      best =
        this.state[this.state.j1best as keyof typeof bestDebater] === null
          ? bestDebater[this.state.j1best as keyof typeof bestDebater]
          : bestDebater[this.state.j1best as keyof typeof bestDebater] +
            ' ' +
            this.state[this.state.j1best as keyof typeof bestDebater];
    } else if (this.state.j2best === this.state.j3best) {
      best =
        this.state[this.state.j2best as keyof typeof bestDebater] === null
          ? bestDebater[this.state.j2best as keyof typeof bestDebater]
          : bestDebater[this.state.j2best as keyof typeof bestDebater] +
            ' ' +
            this.state[this.state.j2best as keyof typeof bestDebater];
    } else {
      best = '无佳辩';
    }
          ...
          style={
            this.state.schedule.substr(0, 2) === '决赛'
              ...
              : {
                  backgroundColor:
                    color[
                      this.state.schedule.substr(3, 1) as keyof typeof color
                    ],
                }
          }
        >
          <div className={styles.row}>
            <Text
              style={{ fontWeight: 'bold', fontSize: '20px', margin: '4px' }}
            >
          ...
          <div className={styles.row}>
            <Row>
              <Col span={24}>辩题：{this.state.topic}</Col>
            </Row>
          </div>
          <Divider />
          ...
          <div className={styles.row}>
            <div>
              <Text>反方：</Text>
              {this.state.f === null ? '' : this.state.f}
            </div>
          </div>
            ...
            <div className={styles.row}>
              ...
              {this.state.j1win === '' ? (
                ...
                <div className={styles.bestdebater}>
                  {this.state.j1best === '' ? (
                    <Tag color={'pink'}>弃票</Tag>
                  ) : (
                    <Tag
                      color={
                        this.state.j1best.substr(0, 1) === 'z' ? 'teal' : 'blue'
                      }
                      type="ghost"
                    >
                      {
                        bestDebater[
                          this.state.j1best as keyof typeof bestDebater
                        ]
                      }
                    </Tag>
                  )}
              ...
              {this.state.j2win === '' ? (
                ...
                <div className={styles.bestdebater}>
                  {this.state.j2best === '' ? (
                    ...
                    <Tag
                      color={
                        this.state.j2best.substr(0, 1) === 'z' ? 'teal' : 'blue'
                      }
                      type="ghost"
                    >
                      {
                        bestDebater[
                          this.state.j2best as keyof typeof bestDebater
                        ]
                      }
                    </Tag>
                  )}
              ...
              {this.state.j3win === '' ? (
                ...
                <div className={styles.bestdebater}>
                  {this.state.j3best === '' ? (
                    ...
                    <Tag
                      color={
                        this.state.j3best.substr(0, 1) === 'z' ? 'teal' : 'blue'
                      }
                      type="ghost"
                    >
                      {
                        bestDebater[
                          this.state.j3best as keyof typeof bestDebater
                        ]
                      }
                    </Tag>
                  )}
          ...
          <div className={styles.rowRate}>
            <div>
              <Text type="secondary">比分：</Text>
              {rate}
            </div>
            <Paragraph
              copyable={{ content: mediaCopy }}
              style={this.state.rights === '2' ? { display: 'none' } : {}}
            ></Paragraph>
          </div>

          <div className={styles.row}>
            <div>
              <Text type="secondary">获胜方：</Text>
              {win}
            </div>
          </div>
